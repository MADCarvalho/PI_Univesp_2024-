...
class User(UserMixin, db.Model):
    ...
    registration_data = db.relationship(
        "Registration_data", uselist=False, backref="user", lazy=True
    )
    applications = db.relationship(
        "Applications", uselist=False, backref="user", lazy=True
    )
    occurrences = db.relationship(
        "Occurrence", uselist=False, backref="user", lazy=True
    )
...
class Applications(UserMixin, db.Model):
    ...
    @staticmethod
    def get_last_applications(user_id):
        sql = open("static/sql/get_total_applications.sql").read() % user_id
